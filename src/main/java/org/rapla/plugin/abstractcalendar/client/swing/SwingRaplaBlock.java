...
public class SwingRaplaBlock extends RaplaBlock implements SwingBlock {
    ...
    static String FOREGROUND_COLOR_REQUEST = AWTColorUtil.getHexForColor(AWTColorUtil.getColorForHex("#b80802"));
    ...
    class RaplaBlockView extends JComponent implements MouseInputListener {
        ...
        public void paint(Graphics g) {
            ...
            if(classtype.equals(DynamicTypeAnnotations.VALUE_CLASSIFICATION_TYPE_AVAILABILITY))
            	paint(g, 100, dim.height); //100 war Standard zu 1000 geändert und 10 --> keine Änderung 
            else
            	paint(g, 100, dim.height); //dim.width mit 100/80 ersetzt
        }
        
        //hier die Bereite zu ändern wirkt sich nur auf die Breite des Farbbalkens aus
        ...
        public void paint(Graphics g, int width, int height) {
            ...
            Color fg = adjustColor(FOREGROUND_COLOR, alpha);
            Color fgReqeust = adjustColor(FOREGROUND_COLOR_REQUEST, alpha);//(isException() ? Color.white : Color.black);
            g.setColor(fg);
            ...
            if (getBuildContext().isPersonVisible()) {
                ...
                for (Allocatable person : persons) {
                    ...
                    g.setColor(isRequest(person) ? fgReqeust : fg);
            ...
            if (getBuildContext().isResourceVisible()) {
                ...
                for (Allocatable resource : resources) {
                    ...
                    g.setColor(isRequest(resource) ? fgReqeust : fg);
                    y = drawString(g, text, y, 7, true);
                }
            }
        }
